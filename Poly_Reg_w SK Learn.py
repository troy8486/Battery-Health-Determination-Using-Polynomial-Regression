...
def PolynomialRegression(X ,Y):
    poly_reg = PolynomialFeatures(degree = 5) # Degree of Polynomial Regression = 8
    X_poly = poly_reg.fit_transform(X) # Transforming X
    lin_reg = LinearRegression()
    lin_reg.fit(X_poly, Y)
    Y_pred = lin_reg.predict(X_poly)
    return X_poly, Y_pred, lin_reg
...
def main():
    ...
    Y = np.array(normalizedSoC).reshape(-1,1)
    
    X_poly, Y_pred, lin_reg = PolynomialRegression(X, Y)

    rsquare = lin_reg.score(X_poly, Y) # Calculating R Square Error
    print("R Square Value =", rsquare)

    mae = mean_absolute_error(Y, Y_pred) # Calculating Mean Absolute Error
    print("Mean Absolute Error =", mae)
   
    mse = MSE(Y, Y_pred) # Calculating Mean Square Error
    rmse = sqrt(mse) # Calculating Root Mean Square Error
